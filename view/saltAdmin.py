...
class Status:
	...
	p = web.input()
        ...
        msl = local.cmd('*',['grains.items','cmd.run','test.ping'],[[],['cat /proc/uptime /proc/loadavg | xargs echo'],[]])
        x = 1
        rt = ""
        for i in msl:
            #print i,msl
...
class Minion_status:
    ...
    def GET(self):
        ...
        s = wheel.call_func('key.list_all')
        minions_rejected = s['minions_rejected']
        minions_denied = s['minions_denied']
        minions_pre = s['minions_pre']
        minions = s['minions']
        #print minions,'2'
        #return render.salt_minion(ShowName=ShowName,uid=SID)
        return render.salt_miniontest(ShowName=ShowName,uid=SID,minions_rejected=minions_rejected,minions_denied=minions_denied,minions_pre=minions_pre,minions=minions)
...
class Cmd:
    CMD = [
        'cmd.run', 'cmd.script', 'cp.get_dir', 'cp.get_file', 'cp.get_url', 
        'cron.list_tab', 
        'disk.usage',
